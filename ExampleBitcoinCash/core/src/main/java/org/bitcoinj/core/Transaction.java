...
public class Transaction extends ChildMessage {
    ...
    public TransactionInput addSignedInput(TransactionOutPoint prevOut, Script scriptPubKey, ECKey sigKey,
        ...
        Sha256Hash hash = forkId
                ? hashForSignatureWitness(inputs.size() - 1, scriptPubKey, prevOut.getConnectedOutput().getValue(),
     ...
     * types and contracts, it can be necessary. When signing a Witness output the
     * scriptCode should be the script encoded into the scriptSig field, for normal
     * transactions, it's the scriptPubKey of the output you're signing for. (See
        ...
        try {
            ...
            if (type != SigHash.SINGLE && type != SigHash.NONE) {
                ...
                for (int i = 0; i < this.outputs.size(); ++i) {
                    uint64ToByteStreamLE(BigInteger.valueOf(this.outputs.get(i).getValue().getValue()), bosHashOutputs);
                ...
                ByteArrayOutputStream bosHashOutputs = new UnsafeByteArrayOutputStream(256);
                uint64ToByteStreamLE(BigInteger.valueOf(this.outputs.get(inputIndex).getValue().getValue()),
                        bosHashOutputs);
